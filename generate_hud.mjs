#!/usr/bin/env node
/**
 * WeaveMap Cross-Platform HUD Generator & Schema Validator
 * 
 * Generates a self-contained, live AI observer HUD artifact for Antigravity,
 * Cursor, VS Code sidebars, and standalone browser viewing.
 * 
 * Zero external dependencies — runs on standard Node.js (Windows, macOS, Linux).
 */
...
const color = (name, str) => useColors ? `${ANSI[name] || ''}${str}${ANSI.reset}` : str;

function printHelp() {
  console.log(`
${color('bold', 'WeaveMap Cross-Platform HUD Generator & Validator')}

${color('cyan', 'Usage:')}
  node generate_hud.mjs [options]
  node generate_hud.mjs <projectPath> <artifactPath>
...
${color('cyan', 'Examples:')}
  node generate_hud.mjs -p . -a ./artifacts/weavemap_hud.html
  node generate_hud.mjs --check-only
`);
}
...
function parseArgs(args) {
  ...
  if (positional.length >= 1 && options.projectPath === '.') {
    options.projectPath = positional[0];
  }
  if (positional.length >= 2 && !options.artifactPath) {
...
/**
 * Inlined HUD CSS, Top Switch Bar, and Client-Side Script
 */
const HUD_CSS = `
/* =========================================================================
    ...
    let html = \`
      <div class="hud-metrics-card">
        ...
        <div class="hud-stat-chips">
          <div class="hud-stat-chip human" data-tooltip="Human Approval Gates: Tasks paused waiting for explicit human review and sign-off. The AI will not proceed without approval.">
      ...
      <!-- 6. The Weave Canvas Viewer (Always last section!) -->
...
async function main() {
  ...
  let weaveDir = path.resolve(options.projectPath, 'weavemap');
  if (!fs.existsSync(path.join(weaveDir, 'index.html'))) {
    weaveDir = path.resolve(options.projectPath);
  }

  const indexPath = path.join(weaveDir, 'index.html');
  const stylePath = path.join(weaveDir, 'style.css');
  ...
  if (validation.warnings.length > 0) {
    for (const warn of validation.warnings) {
      console.warn(color('yellow', `[WARN] `) + warn);
    }
  }

  if (!validation.valid) {
    console.error(color('red', `\nSchema Validation Failed (${validation.errors.length} errors):`));
  ...
  console.log(color('green', `✔ `) + `WeaveMap state valid: ${validation.taskCount} tasks (${validation.doneCount} completed, DAG acyclic)`);

  if (options.checkOnly) {
    console.log(color('cyan', `Validation check complete (--check-only passed).`));
    process.exit(0);
  }

  // 5. Determine destination path
  let destPath = options.artifactPath;
  if (!destPath) {
    destPath = path.join(process.cwd(), 'weavemap_hud.html');
  } else if (!destPath.toLowerCase().endsWith('.html')) {
    destPath = path.join(destPath, 'weavemap_hud.html');
  }

  const parentDir = path.dirname(path.resolve(destPath));
...
main().catch(err => {
  console.error(color('red', 'Unexpected generator error:'), err);
  process.exit(1);
});
